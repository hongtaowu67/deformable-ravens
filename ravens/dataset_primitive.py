...
class DatasetPrimitive:
    ...
    def random_sample(self, primitive_idx, goal_images=False):
        ...
        assert primitive_idx in self.primitive_idx_set
        ...
        if permissible_flag == 0:
            ...
            while True:
                ...
                random = episode_primitive[i][3]['random']
                if not random:
                    break
            obs = {}
            obs['color'] = episode_primitive[i][0]['color']
            obs['depth'] = episode_primitive[i][0]['depth']
            act = {}
            act['params'] = episode_primitive[i][1]
            act['camera_config'] = CAMERA_CONFIG
            act['primitive'] = 'pick_place'
            info = episode_primitive[i][3]
            ...
            act = {}
            act['params'] = {}
            act['params']['pose0'] = episode[i][1]['pose0']
            act['params']['pose1'] = episode[i][1]['pose0']
            act['camera_config'] = CAMERA_CONFIG
            act['primitive'] = 'pick_place'
            info = episode[i][3]
            info['primitive'] = primitive_idx
            assert obs['color'].shape == (3, 480, 640, 3), obs['color'].shape
            assert obs['depth'].shape == (3, 480, 640), obs['depth'].shape
            
        print(f'dataset: {self.path_list[dataset_id]} episode_id: {iepisode} primitive: {primitive_idx} permissible: {permissible_flag}')
        ...
        if goal_images:
            ...
            return obs, act, info, goal, permissible_flag
